...
import { Tokens } from './Reportes/Tokens';
import { Tokenn } from './Reportes/Tokenn';
import * as Narbol from './NodoArbol/Nodo';
import fss = require('fs'); 
...
app.post('/Analizar/', function (req, res) {
   ...
   // consola="";
    ...
    if(Errores.Vacio()){
        ...
        res.json({arbol: tree1, Rerror: Errores.mostrar_Lista().toString(), Reporte_uno: consola, Reporte_dos: Tokens.mostrar_Lista().toString(),ast:contenido});
        ...
        Tokenss(resultado);
            ...
            if(req.body.text2.toString()=="uno"){
               ...
               //recorrer_tree_uno(resultado);
                ...
                res.json({arbol: tree1, Rerror: "nada", Reporte_uno: consola, Reporte_dos: Tokens.mostrar_Lista().toString(),ast:contenido});
...
function Tokenss(Nodos:Narbol.Nodo){
   ...
   try {
    ...
    if((Nodos.descripcion.toString()!="")){
     ...
     Tokens.add(new Tokenn(Nodos.tipo,Nodos.descripcion.toString(), Nodos.fila, Nodos.columna));
...
function AST(Nodos:Narbol.Nodo){
    var padre = "nodo"+i;
...
//}
  ...
  function graficar(Nodos:Narbol.Nodo){
      ...
      try{
        ...
        fss.writeFile(nombre,contenido,(err) => {
            if (err) throw err;
